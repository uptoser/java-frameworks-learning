...
public class C2Main {
	...
	/**
	 * 当前的Redis支持6种数据类型，
	 * 它们分别是字符串（String）、列表（List）、集合（set）、哈希结构（hash）、有序集合（zset）和基数（HyperLogLog）
	 */
...
//		testHyperLogLog();
	...
	/**
	 * 在实际的项目中，虽然阻塞可以有效保证了数据的一致性，但是阻塞就意味着其他进程的等待，
	 * CPU需要给其他线程挂起、恢复等操作，更多的时候我们希望的并不是阻塞的处理请求，
	 * 所以这些命令在实际中使用得并不多
	 */
	...
	/**
	 * Redis的集合不是一个线性结构，而是一个哈希表结构，它的内部会根据hash 分子来存储和查找数据
	 */
